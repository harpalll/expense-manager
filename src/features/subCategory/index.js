...
const Actions = ({ subCategory }) => {
  ...
  const handleToggleStatus = () => {
    dispatch(
      openModal({
        ...
        extraObject: {
          message: `Are you sure you want to toggle ${subCategory.subCategoryName}'s status?`,
          onConfirm: async () => {
            dispatch(toggleSubCategoryStatus(subCategory.subCategoryID))
              .unwrap()
              .then(() => dispatch(fetchSubCategory()))
  ...
  const handleEdit = async (subCategoryID) => {
    try {
      //   const response = await dispatch(fetchCategoryById(subCategoryID));
      //   const categoryData = response.payload;

      dispatch(
        openModal({
          title: "Edit Sub Category",
          bodyType: MODAL_BODY_TYPES.ADD_EDIT_SUB_CATEGORY,
          extraObject: {
            mode: "edit",
...
const TopSideButtons = () => {
  ...
  const openAddNewSubCategoryModal = () => {
    dispatch(
      openModal({
        bodyType: MODAL_BODY_TYPES.ADD_EDIT_SUB_CATEGORY,
  ...
  return (
    <div className="inline-block float-right">
      <button
        ...
        onClick={() => openAddNewSubCategoryModal()}
      >
        Add New
      </button>
    </div>
            ...
            {subCategory?.length === 0 ? (
              <>
                <div className="flex justify-center items-center">
                  <h1>No Sub Categories, click on add new to get started.</h1>
                </div>
              </>
            ) : (
              <>
